...
def read_checksum(path: Path, expected_suffix: str) -> str:
    if not path.is_file() or path.is_symlink():
        raise SbomError(f"Archive checksum must be a regular file: {path}")
    lines = [
        line for line in path.read_text(encoding="utf-8").splitlines() if line.strip()
    ]
    if len(lines) != 1:
        raise SbomError("Archive checksum file must contain exactly one record")
    ...
    return require_sha256(fields[0], f"{expected_suffix} checksum")


def read_provenance(path: Path) -> list[dict[str, str]]:
    if not path.is_file() or path.is_symlink():
...
def build_sbom(
    ...
    archive_sha256 = read_checksum(checksum_path, ".tar.gz")
    release_sha256 = read_checksum(release_checksum_path, ".zip")
    rows = read_provenance(provenance_path)
